...
import { initGameObject, InjectionKeys } from 'phavuer/src/index.js'
import { gameObjectProps, mapProps } from 'phavuer/src/props.js'
import { gameObjectEmits } from 'phavuer/src/emits.js'
...
export default defineComponent({
  emits: [...gameObjectEmits, ...SPRITE_EMITS.map((v) => v.emit)],
  props: {
    ...gameObjectProps,
    ...mapProps('texture', 'frame', 'tint', 'flipX', 'flipY'),
    play: { type: [String, Object] },
  },
  setup(props, context) {
    const scene = inject(InjectionKeys.Scene)
    const object = new Phaser.Physics.Arcade.Sprite(scene, props.x || 0, props.y || 0, props.texture)
